...
const currentReviewShowcase = ref(null);
...
const fetchShowcases = async (page = 1, pageSize = 20, search = '') => {
  ...
  try {
    ...
    const response = await request.get(`${API_BASE_URL}/pending-review?${params}`);
    ...
    if (response.data.code === 200) {
      ...
      message.error(response.data.message || '获取待审核作品列表失败');
    }
  } catch (error) {
    console.error('Failed to fetch showcases:', error);
    message.error('获取待审核作品列表失败');
  } finally {
    loading.value = false;
  }
};
...
// 通过作品
const approveShowcase = (record) => {
  currentReviewShowcase.value = record;
...
const submitReview = async () => {
  if (reviewAction.value === 'reject' && !reviewComment.value.trim()) {
    message.error('拒绝作品时必须填写原因');
    return;
  }

  reviewLoading.value = true;
  try {
    const response = await request.post(`${API_BASE_URL}/${currentReviewShowcase.value.uuid}/review`, {
      action: reviewAction.value,
      review_comment: reviewComment.value.trim() || null
    });

    if (response.data.code === 200) {
      message.success(`作品${reviewAction.value === 'approve' ? '审核通过' : '审核拒绝'}成功`);
...
<template>
  <div class="showcase-management">
    <div class="page-header">
      ...
      <p class="page-subtitle">管理论坛帖子和回复内容</p>
    </div>
      ...
      <a-table-column key="action" title="操作" width="250" fixed="right">
        <template #default="{ record }">
          <div class="action-buttons">
            ...
            <a-button size="small" type="primary" @click="approveShowcase(record)">通过</a-button>
            <a-button size="small" danger @click="rejectShowcase(record)">拒绝</a-button>
          </div>
        </template>
      </a-table-column>
    </a-table>
    ...
    <a-modal
      ...
      width="500px"
    >
      <div v-if="currentReviewShowcase" class="review-modal-content">
        <a-descriptions :column="1" bordered style="margin-bottom: 16px;">
          <a-descriptions-item label="作品名称">
            {{ currentReviewShowcase.name }}
          </a-descriptions-item>
          <a-descriptions-item label="作品简介">
            {{ currentReviewShowcase.summary || '-' }}
          </a-descriptions-item>
          <a-descriptions-item label="创建时间">
            {{ new Date(currentReviewShowcase.created_at).toLocaleString('zh-CN') }}
          </a-descriptions-item>
        </a-descriptions>
        
        <a-form layout="vertical">
          <a-form-item 
            :label="reviewAction === 'approve' ? '审核意见（可选）' : '拒绝原因（必填）'"
...
.showcase-management {
  ...
  height: calc(100vh - 120px); /* 减去导航栏高度和额外间距 */
  display: flex;
