...
import { SearchOutlined, PlusOutlined, SettingOutlined, PlayCircleOutlined, VideoCameraOutlined, UploadOutlined } from '@ant-design/icons-vue';
...
const fetchResources = async (page = 1, pageSize = 20, search = '') => {
  ...
  try {
    ...
    if (response.data.code === 200) {
      ...
      message.error(response.data.message || '获取视频列表失败');
    }
  } catch (error) {
...
const deleteResource = (uuid) => {
  ...
  Modal.confirm({
    title: '确认删除视频',
    content: `确定要删除视频 "${resource.name}" 吗？此操作不可恢复！`,
    okType: 'danger',
    onOk: async () => {
      try {
...
const handleRemoveFile = (file) => {
  ...
  addResourceForm.mime_type = 'video/mp4';
  uploadProgress.value = 0;
  return true;
};
...
const handleCustomUpload = async (options) => {
  ...
  try {
    ...
    const fileKey = `video/${timestamp}_${randomStr}.${fileExtension}`;
    
    const tokenInfo = await getQiniuUploadToken(fileKey, `视频文件上传: ${file.name}`);
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status === 200) {
        ...
        message.error(`视频文件上传失败: HTTP ${xhr.status}`);
        fileList.value = [];
        if (onError) {
    ...
    message.error(error.message || '视频文件上传失败');
    isUploading.value = false;
    uploadProgress.value = 0;
    fileList.value = [];
    if (onError) {
...
const handleCustomCoverUpload = async (options) => {
  ...
  try {
    ...
    const randomStr = Math.random().toString(36).substring(2, 8);
    const fileExtension = file.name.split('.').pop();
    const fileKey = `video/cover/${timestamp}_${randomStr}.${fileExtension}`;
    
    const tokenInfo = await getQiniuUploadToken(fileKey, `视频封面图上传: ${file.name}`);
...
const handleEditCustomUpload = async (options) => {
  ...
  try {
    ...
    const tokenInfo = await getQiniuUploadToken(fileKey, `视频文件编辑上传: ${file.name}`);
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status === 200) {
        try {
          ...
          console.error('解析编辑视频文件响应失败:', parseError);
          message.error('服务器响应格式错误');
          editFileList.value = [];
          if (onError) {
            onError(parseError);
        ...
        console.error('编辑视频文件上传失败:', xhr.status);
        message.error(`视频文件上传失败: HTTP ${xhr.status}`);
        editFileList.value = [];
        if (onError) {
          onError(new Error(`HTTP ${xhr.status}`));
    ...
    xhr.addEventListener('error', (error) => {
      console.error('编辑视频文件上传网络错误:', error);
      message.error('视频文件上传失败');
      isEditUploading.value = false;
      editUploadProgress.value = 0;
      editFileList.value = [];
    ...
    console.error('编辑视频文件上传失败:', error);
    message.error(error.message || '视频文件上传失败');
    isEditUploading.value = false;
    editUploadProgress.value = 0;
    editFileList.value = [];
...
const handleEditCoverBeforeUpload = (file) => {
  return handleCoverBeforeUpload(file);
};
...
const handleEditCustomCoverUpload = async (options) => {
  ...
  try {
    ...
    const tokenInfo = await getQiniuUploadToken(fileKey, `编辑视频封面图上传: ${file.name}`);
...
<template>
  <div class="resource-management">
    <div class="page-header">
      ...
      <p class="page-subtitle">查看、编辑系统中的视频课件</p>
    </div>
          ...
          <template v-if="column.key === 'cover_url'">
            <div class="video-cover-preview">
              <div class="video-thumbnail">
                <img v-if="record.cover_url" :src="record.cover_url" alt="视频封面" />
                <div v-else class="cover-placeholder">
                  <VideoCameraOutlined style="font-size: 32px; color: #ccc;" />
                </div>
                <div class="video-play-overlay">
                  <PlayCircleOutlined style="font-size: 24px; color: white;" />
                </div>
              </div>
            </div>
          </template>
    ...
    >
      <a-form :model="editForm" layout="vertical">
        ...
        <a-form-item label="视频描述">
          <a-textarea v-model:value="editForm.description" placeholder="请输入视频描述" :rows="3" />
        </a-form-item>
      </a-form>
    ...
    >
      <a-form :model="addResourceForm" layout="vertical">
        ...
        <a-form-item label="视频描述">
          <a-textarea v-model:value="addResourceForm.description" placeholder="请输入视频描述" :rows="3" />
        </a-form-item>
      </a-form>
    ...
    >
      <div v-if="resourceDetail" class="resource-detail">
        <a-descriptions :column="1" bordered>
          ...
          <a-descriptions-item label="播放次数">
          ...
          <a-descriptions-item label="封面预览" v-if="resourceDetail.cover_url">
            <div class="detail-video-preview">
              <img :src="resourceDetail.cover_url" alt="视频封面" />
              <div class="detail-play-overlay">
                <PlayCircleOutlined style="font-size: 32px; color: white;" />
              </div>
            </div>
          </a-descriptions-item>
        </a-descriptions>
