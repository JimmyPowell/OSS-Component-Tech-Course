...
const fetchShowcaseComments = async (showcaseUuid, page = 1, pageSize = 20) => {
  ...
  try {
    ...
    const showcaseResponse = await request.get(`${API_BASE}/showcases/${showcaseUuid}`);
    ...
    // 获取评论列表
    const response = await request.get(`${API_BASE}/showcase-comments?showcase_id=${showcaseId}&skip=0&limit=100`);
...
const loadCommentReplies = async (commentUuid) => {
  ...
  try {
    const response = await request.get(`${API_BASE}/showcase-comment-replies?comment_uuid=${commentUuid}&skip=0&limit=100`);
...
const deleteShowcaseComment = (commentUuid) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const response = await request.delete(`${API_BASE}/showcase-comments/${commentUuid}`);
...
const deleteCommentReply = (commentUuid, replyUuid) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const response = await request.delete(`${API_BASE}/showcase-comment-replies/${replyUuid}`);
