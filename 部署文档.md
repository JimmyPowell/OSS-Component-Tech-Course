...
后端已支持通过环境变量集中配置 CORS。请在 `backend/.env` 中设置：

```
CORS_ALLOW_ORIGINS="https://www.opendevcourse.com,https://admin.opendevcourse.com,http://localhost:5173,http://127.0.0.1:5173,http://localhost:5174,http://127.0.0.1:5174"
```

说明：
- 多个域名用英文逗号分隔。
- 开发环境可保留本地 Vite 端口（5173、5174）。
- 不建议在生产使用 `*` 通配所有来源。
...
已改为通过 Vite 环境变量集中管理 API 基础地址：

1) 设置生产环境变量（文件：`frontend/.env.production`）
```
VITE_API_BASE_URL=https://backend.opendevcourse.com/api/v1
```

2) 本地开发（文件：`frontend/.env.development`）
```
...
同样通过 Vite 环境变量集中管理 API 基础地址：

1) 生产环境（文件：`frontend-admin/.env.production`）
```
VITE_API_BASE_URL=https://backend.opendevcourse.com/api/v1
```

2) 本地开发（文件：`frontend-admin/.env.development`）
```
VITE_API_BASE_URL=http://localhost:8000/api/v1
```
